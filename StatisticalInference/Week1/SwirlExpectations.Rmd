...
For a discrete random variable X with PMF p(x), `E(X)` is defined as a sum, over all possible values x, of the quantity
...
Knowing that the expected value is the area under the triangle, `t*f(t)`, what is the expected value of the
  random variable with this PDF? Triangle area. b*h/2
```r
1: .25
2: 2.0
3: 1.0
